...
cmake -G %GENERATOR% -S %CC_MQTT5_GENERATED_SRC_DIR% -B %CC_MQTT5_GENERATED_BUILD_DIR% -DCMAKE_INSTALL_PREFIX=%CC_MQTT5_GENERATED_INSTALL_DIR% -DCMAKE_BUILD_TYPE=%COMMON_BUILD_TYPE% -DOPT_REQUIRE_COMMS_LIB=OFF -DCMAKE_PREFIX_PATH=%COMMS_INSTALL_DIR% -DCMAKE_CXX_STANDARD=%COMMON_CXX_STANDARD%
if %errorlevel% neq 0 exit /b %errorlevel%
cmake --build %CC_MQTT5_GENERATED_DIR% --config %COMMON_BUILD_TYPE% --target install
if %errorlevel% neq 0 exit /b %errorlevel%
